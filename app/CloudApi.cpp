...
std::string
CloudApi::detectImage_curl_hard()
{
    CURLcode ret;
    CURL *curl;
...
std::string
CloudApi::detectImage_hard()
{
    return detectImage_curl_hard();
    ...
    header.push_back("Authorization: "
                     ...
                     "eyJqdGkiOiIyOTcyMDg2NC1jZDg5LTQ3MGEtYWUzYS1m"
                     "OWYzOWY1OWNiODYiLCJzdWIiOjM2LCJpYXQiOjE1NjIx"
                     "Nzg0NjYsIm5iZiI6MTU2MjE3ODQ2NiwidHlwZSI6ImFj"
                     "Y2VzcyIsImZyZXNoIjpmYWxzZX0.TSwvRkY0cIOHNU"
                     "-copIcfWiDdia7EnEoq1MD51i2jG8");
    ...
    //    curlpp::options::ReadFile()
    //    request.setOpt(new curlpp::options::ReadFile(file));
    request.setOpt(new curlpp::options::Verbose(true));
    request.setOpt(new curlpp::options::HttpHeader(header));
    request.setOpt(new curlpp::options::PostFields(const_cast<char*>(image_buff)));
    request.setOpt(new curlpp::options::PostFieldSize(int(file_size)));
    request.setOpt(new curlpp::options::Url(
                       "https://backend.facecloud.tevian.ru/api/v1/detect"
                       "?fd_min_size=0&fd_max_size=0&fd_threshold=0.8"
                       "&demographics=true&attributes=true&landmarks=false"));
